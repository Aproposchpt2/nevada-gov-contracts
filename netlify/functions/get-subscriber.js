...
exports.handler = async (event) => {
  ...
  const r = await fetch(`${SUPABASE_URL}/rest/v1/state_alert_subscribers?session_token=eq.${encodeURIComponent(token)}&session_expires_at=gt.${encodeURIComponent(nowIso)}&select=email,business_name,state,keywords,alerts_opt_in,alert_cadence,alert_days,stripe_customer_id&limit=1`, { headers: sbH() });
