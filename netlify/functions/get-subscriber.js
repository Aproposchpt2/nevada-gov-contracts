...
exports.handler = async (event) => {
  ...
  const r = await fetch(`${SUPABASE_URL}/rest/v1/state_alert_subscribers?session_token=eq.${encodeURIComponent(token)}&session_expires_at=gt.${encodeURIComponent(nowIso)}&select=email,business_name,state,keywords,alerts_opt_in&limit=1`, { headers: sbH() });
  const rows = await r.json();
  if (!Array.isArray(rows) || !rows.length) return j(401, { error: 'Your session expired. Please log in again.' });
  const s = rows[0];
  return j(200, { ok: true, email: s.email, business_name: s.business_name, state: s.state, keywords: s.keywords || [], alerts_opt_in: s.alerts_opt_in });
};
