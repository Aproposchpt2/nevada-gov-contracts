...
exports.handler = async (event) => {
  ...
  if (RESEND_KEY) {
    ...
    await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: 'Bearer ' + RESEND_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: FROM, to: [email], subject: `Your NV Gov Contracts login code: ${code}`, html }) }).catch(e => console.error('[send-login-code]', e.message));
  }
