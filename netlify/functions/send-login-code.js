...
exports.handler = async (event) => {
  ...
  if (RESEND_KEY) {
    const html = `<div style="background:#0F2A6A;padding:34px 16px;font-family:Arial,sans-serif"><div style="max-width:460px;margin:0 auto;background:#11264f;border:1px solid #1c3878;border-radius:8px;padding:32px;text-align:center"><div style="font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:#9DB0D4;margin-bottom:14px">StateGen · Member Login</div><p style="color:#9DB0D4;font-size:14px;margin:0 0 18px">Your login code:</p><div style="font-size:34px;letter-spacing:.32em;font-weight:700;color:#fff;background:#07111f;border:2px solid #6EE7A8;border-radius:10px;padding:18px;margin-bottom:18px">${code}</div><p style="color:#9DB0D4;font-size:13px;line-height:1.6">Enter this code to access your dashboard. It expires in 10 minutes.</p></div></div>`;
    await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: 'Bearer ' + RESEND_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: FROM, to: [email], subject: `Your StateGen login code: ${code}`, html }) }).catch(e => console.error('[send-login-code]', e.message));
  }
